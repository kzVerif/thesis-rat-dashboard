...
เชื่อม WebSocket `/ws/frontend` ใช้ session เดียวกับระบบเดิม: browser ส่ง cookie `__Host-session` อัตโนมัติบน HTTPS/WSS ส่วน client ที่กำหนด header ได้ใช้ `Authorization: Bearer <session-token>` ได้ ต้องตั้ง `FRONTEND_ORIGINS` ให้ตรงกับ origin ของหน้าบ้าน (ดูการตั้งค่าใน README) ไม่มีการส่ง token ผ่าน URL

ต้องเป็นผู้ใช้ ACTIVE มี session ที่ยังไม่หมดอายุ/ไม่ถูก revoke ตอนเชื่อมต่อ และมี permission `agents.manage` สำหรับคำสั่งสแกนและประวัติ ระบบตรวจ permission และสถานะผู้ใช้อีกครั้งทุกคำสั่ง ประวัติที่เรียกได้เป็นงานที่ผู้ใช้นั้นสั่งเท่านั้น
...
ตัวอย่าง error: `forbidden`, `invalid agent_id`, `invalid request_id`, `scan_type must be quick, full, or custom`, `custom path must be an absolute Windows path`, `cannot create scan`, `cannot load scans` ข้อผิดพลาดการเชื่อมต่อ session ส่ง HTTP 401 ก่อน upgrade ข้อผิดพลาดคำสั่งไม่มี request ID ให้ส่งคำสั่งสร้างทีละคำสั่งรอ accepted/error เพื่อจับคู่ได้ง่าย
